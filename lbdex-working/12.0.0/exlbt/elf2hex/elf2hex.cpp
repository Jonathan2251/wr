...
#define ELF2HEX
#include "../llvm-objdump/llvm-objdump.cpp"
#include "elf2hex.h"
...
static std::error_code getName(llvm::object::SectionRef const &Section, StringRef Name) {
  Expected<StringRef> SecNameOrErr = Section.getName();
  if (!SecNameOrErr) {
    consumeError(SecNameOrErr.takeError());
    reportError(CurrInputFile, "Abort in getName(...)");
  }
  Name = *SecNameOrErr;
  SecNameOrErr.takeError();
}
...
T unwrapOrError(Expected<T> EO, Ts &&... Args) {
  if (EO)
    return std::move(*EO);
  // use above report_error(StringRef File, llvm::Error E)
  report_error(std::forward<Ts>(Args)..., EO.takeError());
}
...
static std::error_code getRelocationValueString(const ELFObjectFile<ELFT> *Obj,
  ...
  Expected<const Elf_Shdr *> SecOrErr = EF.getSection(Rel.d.a);
  if (!SecOrErr)
    return std::error_code();
  const Elf_Shdr *Sec = *SecOrErr;
  Expected<const Elf_Shdr *> SymTabOrErr = EF.getSection(Sec->sh_link);
  if (!SymTabOrErr)
    return std::error_code();
  const Elf_Shdr *SymTab = *SymTabOrErr;
  assert(SymTab->sh_type == ELF::SHT_SYMTAB ||
         SymTab->sh_type == ELF::SHT_DYNSYM);
  Expected<const Elf_Shdr *> StrTabSec = EF.getSection(SymTab->sh_link);
  if (!StrTabSec)
    return std::error_code();
  Expected<StringRef> StrTabOrErr = EF.getStringTable(*StrTabSec);
  if (!StrTabOrErr)
    return std::error_code();
  ...
  if (symb->getType() == ELF::STT_SECTION) {
    ...
    Expected<StringRef> SecName = EF.getSectionName(SymSec);
    if (!SecName)
      return std::error_code();
    Target = *SecName;
  } else {
...
static void printRelocationTargetName(const MachOObjectFile *O,
  ...
  if (isExtern) {
    ...
    advance(SI, Val - 1);
    S = unwrapOrError(SI->getName(), O->getFileName());
  }
